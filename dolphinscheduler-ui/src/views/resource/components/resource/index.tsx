...
import { queryCurrentResourceById, queryResourceById } from "@/service/modules/resources";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    onMounted(() => {
      if (variables.fileId === -1) {
        fileStore.setCurrentDir('/')
        breadListRef.value = ['']
      } else {
        queryCurrentResourceById(variables.fileId)
          .then((res: ResourceFile) => {
              if (res.fullName) {
                fileStore.setCurrentDir(res.fullName)
                breadListRef.value = fileStore.getCurrentDir.split('/') as Array<string>
              }
            }
          )
      }
    ...
    const handleBread = (index: number) => {
      const breadName = '/' + breadListRef.value?.slice(1, index + 1).join('/')
      goBread(breadName)
    }

    const goBread = (fullName: string) => {
      const { fileId, resourceType } = variables
      if (fullName === '/') {
